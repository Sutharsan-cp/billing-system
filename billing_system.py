...
class Bill_App:
    ...
    # =============customer retail details======================
        ...
        cname_txt = Entry(F1, width=12,textvariable=self.c_name, font='arial 15', bd=7, relief=GROOVE)
    ...
    # ===================Medical====================================
        ...
        aspirin_lbl = Label(F2, text="aspirin", font =('times new roman', 16, 'bold'), bg = "#badc57", fg = "black")
        aspirin_lbl.grid(row=4, column=0, padx=10, pady=10, sticky='W')
        aspirin_txt = Entry(F2, width=10, textvariable=self.aspirin, font=('times new roman', 16, 'bold'), bd=5, relief=GROOVE)
        aspirin_txt.grid(row=4, column=1, padx=10, pady=10)
    ...
    # ===========ColdDrinks================================
        ...
        pepsi_lbl.grid(row=1, column=0, padx=10, pady=10, sticky='W')
        pepsi_txt = Entry(F4, width=10, textvariable=self.pepsi, font=('times new roman', 16, 'bold'), bd=5, relief=GROOVE)
        pepsi_txt.grid(row=1, column=1, padx=10, pady=10)
    ...
    # =================BillArea======================
        ...
        bill_title.pack(fill=BOTH,expand=1)
...
#================totalBill==========================
    def total(self):
        ...
        self.m_a_p = self.aspirin.get()*5
        self.m_t_g_p = self.thermal_gun.get()*15
        self.total_medical_price = float(self.m_m_p+self.m_h_g_p+self.m_d_p+self.m_a_p+self.m_t_g_p+self.m_s_p)
        ...
        self.c_d_p_p = self.pepsi.get()*10
        ...
        self.total_cold_drinks_price = float(self.c_d_s_p+self.c_d_p_p+self.c_d_m_p+self.c_d_c_p+self.c_d_f_p+self.c_m_d)
...
#=========billArea=================================================
    def bill_area(self):
        ...
        elif self.medical_price.get() == "" and self.grocery_price.get() == "" and self.cold_drinks_price.get()=="":
            messagebox.showerror("Error", "No Product Purchased")
        else:
            self.welcome_bill()
    # ============medical===========================
        if self.sanitizer.get() != 0:
            self.txtarea.insert(END, "\n Sanitizer\t\t{}\t\t{}".format(self.sanitizer.get(),self.m_s_p))
        if self.mask.get() != 0:
        ...
        if self.aspirin.get() != 0:
            self.txtarea.insert(END, f"\n aspirin\t\t{self.aspirin.get()}\t\t{self.m_a_p}")
    ...
    # ==============Grocery============================
        ...
        if self.pepsi.get() != 0:
            self.txtarea.insert(END, f"\n Sanitizer\t\t{self.pepsi.get()}\t\t{self.c_d_l_p}")
        if self.mazza.get() != 0:
            self.txtarea.insert(END, f"\n Mazza\t\t{self.mazza.get()}\t\t{self.c_d_m_p}")
        if self.coke.get() != 0:
            self.txtarea.insert(END, f"\n Dettol\t\t{self.coke.get()}\t\t{self.c_d_c_p}")
        if self.fanta.get() != 0:
            self.txtarea.insert(END, f"\n Fanta\t\t{self.aspirin.get()}\t\t{self.c_d_f_p}")
    ...
    #=========savebill============================
    def save_bill(self):
        path='D:/BARATH/msc ds/2nd sem/Phython programming lab/bills/'
        op = messagebox.askyesno("Save Bill", "Do you want to save the bill?")
        if op > 0:
            self.bill_data = self.txtarea.get('1.0', END)
            f1 = open(path+str(self.bill_no.get())+".txt", "w")
    ...
    def find_bill(self):
        ...
        for i in os.listdir(path):
            if i.split('.')[0] == self.search_bill.get():
                ...
                f1 = open(f"{path}{self.search_bill.get()}.txt", "r")
                self.txtarea.delete("1.0", END)
                for d in f1:
